...
{
  // Nothing to do here
  fAnalysisType = Types::EAnalysisType(Types::kClassification);
}
...
void TMVA::MethodDAE::ProcessOptions() {
  if (IgnoreEventsWithNegWeightsInTraining()) {             // Shall we keep this?
   ...
   for (auto& block : strategyKeyValues) {
      ...
      double fWeightDecay = 0.0;
      bool isTraining = false;
      net = new TMVA::DNN::TDeepAutoEncoder<Architecture_t>(BatchSize, InputDepth, InputHeight, InputWidth,
...
void TMVA::MethodDAE::Train() {
  ...
  //  }
   ...
   // have to change them
   std::vector<size_t> numHiddenUnitsPerLayer={4,2};
   std::vector<Matrix_t> testInput;
   std::vector<Matrix_t> inputLabel;
   size_t outputUnits;
   size_t testDataBatchSize;

   net->PreTrain(input, numHiddenUnitsPerLayer, learningRate, corruptionLevel, dropoutProbability, epochs, activation, applyDropout);
   net->FineTune(input, testInput, inputLabel, outputUnits, testDataBatchSize, learningRate, epochs);
    }
  }
